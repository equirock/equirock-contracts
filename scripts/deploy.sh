...
TX_HASH=$(echo $KEYPASSWD | injectived tx wasm store artifacts/equirock_contracts.wasm --from howlpack --gas-prices 500000000inj --gas auto --gas-adjustment 1.3 -o json -y | jq '.txhash' -r)
sleep 5
...
TX_HASH=$(echo $KEYPASSWD | injectived tx wasm instantiate $CODE_ID "$MSG" --from $ADMIN --admin $ADMIN --gas-prices 500000000inj --gas auto --gas-adjustment 1.3 --label "$ETF_TOKEN_NAME" -o json -y | jq '.txhash' -r)
sleep 5
echo $TX_HASH
